...
namespace CrudWebApi.Controllers
{
    public class DoctorController : ApiController
    {
        ...
        public HttpResponseMessage Point(string lastName)
        {
            ...
            var doctorName = doctors.FindAll(c => c.LastName == lastName);
            if (doctors == null || doctors.Count == 0)
            {
                return Request.CreateResponse(HttpStatusCode.BadRequest, "There is no doctors at list");
            }
            else if (doctorName.Count == 0)
            {
                return Request.CreateResponse(HttpStatusCode.BadRequest, $"There is no doctor with '{lastName}' last name at list");
            }
            return Request.CreateResponse(HttpStatusCode.OK, doctorName);
        }
        ...
        public HttpResponseMessage Point(int lid)
        {
            ...
            {
                return Request.CreateResponse(HttpStatusCode.BadRequest, $"There is no doctor with Licence:'{lid}' on the list");
            }
        ...
        public HttpResponseMessage Add(DoctorViewModel doctor)
        {
            ...
            return Request.CreateResponse(HttpStatusCode.OK, $"You succsessfully add doctor of {doctor.Specialization} {doctor.FirstName} {doctor.LastName}, with Licence: { doctor.LicenceID} ");
